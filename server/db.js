...
export class Database {
  constructor() {
    this.data = { weekId: '', adminPasswordHash: hashPassword('gameopedia@admin2026'), sheetWebhookUrl: '', chatWebhookUrl: '', users: [], sports: [] };
    this.init();
  }
      ...
      if (fs.existsSync(DB_FILE)) {
        ...
        if (this.data.sports) {
          this.data.sports = this.data.sports.map(s => {
            ...
            return match ? { ...s, image: match.image, gradient: match.gradient, tagline: match.tagline, category: match.category, description: match.description, themeColor: match.themeColor } : s;
          });
          this.save();
        }
        if (this.data.weekId !== nextWeek.weekId) this.resetForWeek(nextWeek.weekId);
      } else {
  ...
  loginGoogleUser(email, name) {
    ...
    if (!user) {
      user = { email: cleanEmail, name: (name || cleanEmail.split('@')[0]).trim(), passwordHash: '', role: 'user', createdAt: new Date().toISOString() };
      this.data.users.push(user);
      this.save();
    ...
    const token = Buffer.from(`google:${user.email}:${Date.now()}`).toString('base64');
    return { user: { email: user.email, name: user.name, role: user.role }, token };
  }
  ...
  loginUser(email, password) {
    ...
    if (!user) return this.registerUser(email, password);
  ...
  updateAdminPassword(newPassword) {
    ...
    this.data.adminPasswordHash = hashPassword(newPassword); this.save(); return true;
  }
  ...
  signup(sportId, { name, email }) {
    const sport = this.getSportById(sportId); if (!sport) throw new Error('Sport not found');
    const cleanEmail = email.trim().toLowerCase(); const cleanName = (name || cleanEmail.split('@')[0]).trim();
    const existing = sport.signups.find(p => p.email.toLowerCase() === cleanEmail);
    if (existing) return { sport, isNew: false };
    sport.signups.push({ name: cleanName, email: cleanEmail, signedUpAt: new Date().toISOString() }); this.save();
    integrations.dispatchSheetSync('SIGNUP', { sportName: sport.name, userName: cleanName, userEmail: cleanEmail, venue: sport.venue, timing: sport.timing, totalSignups: sport.signups.length });
    return { sport, isNew: true, signup: sport.signups[sport.signups.length - 1] };
  }

  cancelSignup(sportId, email) {
    const sport = this.getSportById(sportId); if (!sport) throw new Error('Sport not found');
    const cleanEmail = email.trim().toLowerCase(); const idx = sport.signups.findIndex(p => p.email.toLowerCase() === cleanEmail);
    if (idx === -1) return { sport, removed: false };
    const removed = sport.signups[idx]; sport.signups.splice(idx, 1); this.save();
    integrations.dispatchSheetSync('CANCEL', { sportName: sport.name, userName: removed.name, userEmail: cleanEmail, venue: sport.venue, timing: sport.timing, totalSignups: sport.signups.length });
    return { sport, removed: true };
  }

  updateVenueTiming(sportId, { venue, timing }) {
    const sport = this.getSportById(sportId); if (!sport) throw new Error('Sport not found');
    if (venue !== undefined) sport.venue = venue.trim() || 'TBA'; if (timing !== undefined) sport.timing = timing.trim() || 'TBA'; this.save(); return sport;
  }
  ...
  getStats() {
    const totalSignups = this.data.sports.reduce((acc, s) => acc + (s.signups?.length || 0), 0); const uniquePlayers = new Set();
    const sportRankings = this.data.sports.map(s => { s.signups.forEach(p => uniquePlayers.add(p.email.toLowerCase())); return { id: s.id, name: s.name, icon: s.icon, signupsCount: s.signups.length }; }).sort((a, b) => b.signupsCount - a.signupsCount);
